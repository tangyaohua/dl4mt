...
from data_iteratorlr import TextIterator
...
def build_model(tparams, options, temp_2_targetvocab):
    opt_ret = dict()
    ...
    # cost
    y_flat = temp_2_targetvocab[y.flatten()]
    y_flat_idx = tensor.arange(y_flat.shape[0]) * options['n_wordstemp'] + y_flat
...
# generate sample
def gen_sample(temp_vocab, tparams, f_init, f_next, x, options, trng=None, k=1, maxlen=30,
    ...
    for ii in xrange(maxlen):
        ...
        if stochastic:
            ...
            ranks_flat = cand_flat.argsort()[:(k-dead_k)] # beam search, top k porbs
...
def train(dim_word=100, # word vector dimensionality
    ...
    train = TextIterator(datasets[0], datasets[1], 
                         ...
                         n_words_source=n_words_src, n_words_target=n_words_trg,
                         tau=tau,
                         maxlen=maxlen)
    ...
    for eidx in xrange(max_epochs):
        n_samples = 0
        for source, target, temp_vocab in train:
            n_words=len(temp_vocab)
            model_options['n_wordstemp']=n_words
            temp_vocab=numpy.asarray(list(temp_vocab))
            temp_2_targetvocab=numpy.zeros((n_words_trg), dtype='int32')
            temp_2_targetvocab[temp_vocab]=numpy.arange(n_words)
            ...
            trng, use_noise, \
                  ...
                  opt_ret, \
                  cost = \
                  build_model(sub_params, model_options, temp_2_targetvocab)
            inps = [x, x_mask, y, y_mask]

            print 'Buliding sampler'
            f_init, f_next = build_sampler(sub_params, model_options, trng)
            ...
            if decay_c > 0.:
                ...
                weight_decay = 0.
                for kk, vv in sub_params.iteritems():
            ...
            grads = tensor.grad(cost, wrt=itemlist(sub_params))
            print 'Done'
            ...
            f_grad_shared, f_update = eval(optimizer)(lr, sub_params, grads, inps, cost)
            print 'Done'
            ...
            for index in xrange(len(source)/batch_size):
                x=source[index * batch_size: (index + 1) * batch_size]
                y=target[index * batch_size: (index + 1) * batch_size]
                n_samples += len(x)
                uidx += 1
                ...
                if numpy.mod(uidx, sampleFreq) == 0:
                    ...
                    for jj in xrange(numpy.minimum(5,x.shape[1])):
                        stochastic = True
                        sample, score = gen_sample(temp_vocab, sub_params, f_init, f_next, x[:,jj][:,None],
                                                       model_options, trng=trng, k=1, maxlen=30,
                                                       stochastic=stochastic, argmax=False)
                        sample=temp_vocab[numpy.asarray(sample)]
                        print 'Source ',jj,': ',
                        for vv in x[:,jj]:
                            if vv == 0:
                                break
