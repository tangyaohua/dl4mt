...
from ..trainers.optimize import optimize_updates
...
class NeuralTrainer(object):
    ...
    def __init__(self, network, config=None):
        ...
        self.hiddenoutput_func = theano.function(
            network.input_variables, self.hiddenoutput,
            allow_input_downcast=True, mode=self.config.get("theano_mode", None))
    ...
    def _setup_costs(self):
        ...
        self.hiddenoutput =self.network._hidden_outputs
    ...
    def hidout(self, input):
        first=False

        print self.network.input_variables[0]
        back=self.hiddenoutput_func(input[1])
        print len(back)
        print back[1].shape
        print back
        # for x in input:
        #     back=self.hiddenoutput_func(x[0])
        #     print back
        #     print back.shape
            # print x
            # print self.evaluation_func(*x)
            # if not first:
            #     first=True
                # print x[0][1]
                # print x[0][1].shape
                # back=self.hiddenoutput_func(x[0])[1][1]
                # back=self.hiddenoutput_func(x)
                # print back
                # print back.shape
    def hidout2(self, input):
        first=False
        ...
        # return [self.hiddenoutput_func(x[0]) for x in input]
